...
module.exports = class Transport {
    ...
    async write(data, ops) {
        ...
        if (!~util.inspect(this._writing).indexOf('pending') && this._onFlush)
            this._onFlush();
    }
    async _process(data, ops) {
        if (this.preprocessor)
            [data, ops] = this.preprocessor.then ? await (this.preprocessor(data, ops)) : this.preprocessor(data, ops);
        ...
        const formatted = this._formatter[levelName](data);
        let filtered = this.filters[levelName] ?
            this.filters[levelName](formatted) :
            formatted;

        if (this.postprocessor)
            filtered = this.postprocessor.then ? await (this.postprocessor(filtered)) : this.postprocessor(filtered);
